...
def batch_evaluate_command(args):
    ...
    try:
        ...
        if results:
            print(f"Average Score: {sum(r.total_score for r in results) / len(results):.2%}")
...
def list_command(args):
    ...
    try:
        ...
        if args.format == "json":
            ...
            print(f"{'-'*80}")
            for p in problems:
                print(f"{p['id']:<15} {p['title'][:40]:<40} {p['category']:<15} {p['difficulty']:<10}")
...
def validate_command(args):
    ...
    try:
        ...
        if abs(total_weight - 1.0) > 0.01:
            issues.append(f"Evaluation criteria weights sum to {total_weight}, should be 1.0")
        ...
        if issues:
            ...
            print(f"  - Functional Requirements: {len(problem.functional_requirements)}")
...
def main():
    ...
    # evaluate command
    evaluate_parser = subparsers.add_parser("evaluate", help="Evaluate a single submission")
    evaluate_parser.add_argument("--problem", "-p", required=True, help="Problem ID to evaluate")
    evaluate_parser.add_argument("--submission", "-s", required=True, help="Path to submission")
    evaluate_parser.add_argument("--submission-id", help="Unique submission identifier")
    evaluate_parser.add_argument("--environment", default="docker", choices=["docker", "kubernetes", "local"])
    evaluate_parser.add_argument("--timeout", type=int, default=3600, help="Evaluation timeout in seconds")
    evaluate_parser.add_argument("--working-dir", help="Working directory for evaluation")
    evaluate_parser.add_argument("--output", "-o", help="Output directory for results")
    
    # batch-evaluate command
    batch_parser = subparsers.add_parser("batch-evaluate", help="Evaluate multiple problems")
    batch_parser.add_argument("--submission", "-s", required=True, help="Path to submission")
    batch_parser.add_argument("--difficulty", choices=["basic", "intermediate", "advanced", "expert"])
    batch_parser.add_argument("--category", help="Filter by category")
    batch_parser.add_argument("--environment", default="docker", choices=["docker", "kubernetes", "local"])
    batch_parser.add_argument("--timeout", type=int, default=3600, help="Evaluation timeout per problem")
    ...
    list_parser.add_argument("--difficulty", choices=["basic", "intermediate", "advanced", "expert"])
    list_parser.add_argument("--category", help="Filter by category")
    list_parser.add_argument("--format", choices=["table", "json"], default="table")
    
    # report command
    report_parser = subparsers.add_parser("report", help="Generate evaluation report")
    report_parser.add_argument("--results", "-r", required=True, help="Path to results file or directory")
    report_parser.add_argument("--format", choices=["html", "markdown", "json"], default="html")
    report_parser.add_argument("--output", "-o", help="Output file path")
    
    # validate command
    validate_parser = subparsers.add_parser("validate", help="Validate problem definition")
    validate_parser.add_argument("--problem", "-p", required=True, help="Path to problem YAML file")
