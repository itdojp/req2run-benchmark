...
class Evaluator:
        ...
        try:
            ...
            if not deployment_success:
                return self._create_failed_result(
                    submission_id, "Deployment failed", time.time() - start_time, logs
                )
    ...
    def _deploy(self, submission_path: Path, logs: List[str]) -> bool:
        ...
        logs.append(
            f"[{datetime.now()}] Starting deployment in {self.environment} environment"
        )
        ...
        try:
            ...
            if build_result.status.value != "success":
                logs.append(
                    f"[{datetime.now()}] Build failed: {build_result.error_message}"
                )
            ...
            deployment_config = DeploymentConfig(
                ...
                environment={
                    ...
                    "NODE_ENV": "production",
                    "DATABASE_URL": (
                        self.working_dir / "test.db"
                        if "database" in str(self.problem.deployment_requirements)
                        else ""
                    ),
                },
                resources={"memory": "2g", "cpu_quota": 200000},  # 2 CPUs
                healthcheck={
                    "endpoint": "/health",
                    "port": 3000,
                    "timeout": 30,
            ...
            if "ports" in self.problem.deployment_requirements:
                ...
                if isinstance(ports, list) and ports:
                    ...
                    if len(port_mapping) == 2:
                        deployment_config.ports = {
                            int(port_mapping[1]): int(port_mapping[0])
                        }
            ...
            if deployment_result.status.value != "ready":
                logs.append(
                    f"[{datetime.now()}] Deployment failed: {deployment_result.error_message}"
                )
    ...
    def _run_tests(self, logs: List[str]) -> List[TestCase]:
        ...
        for test_case in self.problem.test_cases:
            logs.append(
                f"[{datetime.now()}] Running test: {test_case.id} - {test_case.description}"
            )
        ...
        try:
            ...
            if "status" in expected:
                if response.status_code != expected["status"]:
                    logs.append(
                        f"Status mismatch: expected {expected['status']}, got {response.status_code}"
                    )
                    return False
            ...
            if "body_contains" in expected:
                try:
                    ...
                    for key, value in expected["body_contains"].items():
                        if key not in actual_body or actual_body[key] != value:
                            logs.append(
                                f"Body missing expected key '{key}' with value '{value}'"
                            )
                            return False
                except:
                    logs.append(f"Failed to parse response as JSON")
    ...
    def _execute_cli_test(self, test_case: TestCase, logs: List[str]) -> bool:
        ...
        try:
            ...
            result = self.runner.execute(
                command, timeout=30, container_id=self.container_id
            )
            ...
            if "exit_code" in expected:
                if result.exit_code != expected["exit_code"]:
                    logs.append(
                        f"Exit code mismatch: expected {expected['exit_code']}, got {result.exit_code}"
                    )
                    return False
        ...
        try:
                ...
                if req.get("type") == "performance":
                    ...
                    if "req/min" in constraint:
                        ...
                        if match:
                            ...
                            test_config["users"] = max(
                                10, req_per_min // 6
                            )  # Approximate

            # Run performance test
            logs.append(
                f"[{datetime.now()}] Running performance test with {test_config['users']} users"
            )
            ...
            for req in self.problem.non_functional_requirements:
                if req.get("type") == "performance":
                    ...
                    if (
                        "パーセンタイルレスポンス時間" in constraint
                        or "response time" in constraint.lower()
                    ):
                        ...
                        if match:
                            ...
                            if metrics.response_time_p95 > target_ms:
                                ...
                                logs.append(
                                    f"[{datetime.now()}] P95 response time {metrics.response_time_p95}ms exceeds target {target_ms}ms"
                                )
                    ...
                    if "throughput" in constraint.lower():
                        ...
                        if match:
                            ...
                            if metrics.requests_per_second < target_rps:
                                ...
                                logs.append(
                                    f"[{datetime.now()}] Throughput {metrics.requests_per_second} req/s below target {target_rps} req/s"
                                )
            ...
            if metrics.error_rate > 0.05:  # More than 5% errors
                ...
                logs.append(
                    f"[{datetime.now()}] High error rate: {metrics.error_rate*100:.1f}%"
                )

            logs.append(f"[{datetime.now()}] Performance test completed")
            logs.append(
                f"[{datetime.now()}] - Requests/sec: {metrics.requests_per_second:.1f}"
            )
            logs.append(
                f"[{datetime.now()}] - P95 latency: {metrics.response_time_p95:.1f}ms"
            )
            logs.append(
                f"[{datetime.now()}] - Error rate: {metrics.error_rate*100:.1f}%"
            )
    ...
    def _run_security_scan(self, logs: List[str]) -> float:
        ...
        try:
            ...
            logs.append(
                f"[{datetime.now()}] Running {scan_config['tool']} security scan"
            )
            security_metrics = calculator.run_security_scan(
                submission_path, scan_config
            )

            # Log findings
            total_vulns = (
                security_metrics.critical_vulnerabilities
            ...
            logs.append(
                f"[{datetime.now()}] - Critical: {security_metrics.critical_vulnerabilities}"
            )
            logs.append(
                f"[{datetime.now()}] - High: {security_metrics.high_vulnerabilities}"
            )
            logs.append(
                f"[{datetime.now()}] - Medium: {security_metrics.medium_vulnerabilities}"
            )
            logs.append(
                f"[{datetime.now()}] - Low: {security_metrics.low_vulnerabilities}"
            )
            logs.append(
                f"[{datetime.now()}] Security score: {security_metrics.security_score:.2f}"
            )
    ...
    def _analyze_code_quality(self, submission_path: Path, logs: List[str]) -> float:
        ...
        try:
            ...
            quality_metrics = calculator.run_code_analysis(
                submission_path, analysis_config
            )
            ...
            if quality_metrics.lines_of_code > 0:
                logs.append(
                    f"[{datetime.now()}] - Lines of code: {quality_metrics.lines_of_code}"
                )
            if quality_metrics.cyclomatic_complexity > 0:
                logs.append(
                    f"[{datetime.now()}] - Avg complexity: {quality_metrics.cyclomatic_complexity:.1f}"
                )
            if quality_metrics.test_coverage >= 0:
                logs.append(
                    f"[{datetime.now()}] - Test coverage: {quality_metrics.test_coverage*100:.1f}%"
                )
            if quality_metrics.linting_score > 0:
                logs.append(
                    f"[{datetime.now()}] - Linting score: {quality_metrics.linting_score:.2f}"
                )
    ...
    def _calculate_final_result(
        ...
        validated_reqs = sum(
            1 for req in self.problem.functional_requirements if req.validated
        )
        ...
        for criterion in self.problem.evaluation_criteria:
            if "機能" in criterion.metric or "functional" in criterion.metric.lower():
                ...
                "性能" in criterion.metric or "performance" in criterion.metric.lower()
            ):
                score = performance_score
            elif (
                "セキュリティ" in criterion.metric
                or "security" in criterion.metric.lower()
            ):
